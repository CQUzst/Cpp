...
void IncPIDInit(void)
{
	...
	sptr->Proportion = 1;//0.02;  //比例常数 Proportional Const
	sptr->Integral = 0.001;//0.001;   //积分常数 Integral Const
...
void DrawLine()
{
	...
	Uint16 showSetSpeed=MotorADJ.Set_Speed+1000;
	HexPrintf(showSetSpeed);
	CmdT[5] = (showSetSpeed)>>8;
	CmdT[6] = (showSetSpeed)&0XFF;
	//发送CRC
	...
	case M_SPEEDLOOP://速度闭环模式
		if (Mark.Status_Bits.MotorState != M_STOP) //电机运行命令使能
		{
			...
			else if (MotorADJ.OpenLoopSetUpCNT == 0) //电机启动之后，进行速度闭环运算
			{
				...
				if(MotorADJ.CloseSPLoopCnt < 1)//P环启动
				{
		...
		if(SCI_Msg.timerOut >= RTU_TIMEROUT) 	  //判断接收完成
		{
			...
			case 0x04:
				if(Mark.Status_Bits.MotorState == M_STOP)//控制反向
				{
					...
					{
						sciPutString(1,"电机反转",1);
						HallEncode.HallType = M_CW;
						D400LOW();
					}
					else
					{
						sciPutString(1,"电机正转",1);
			...
			case 0x05://设置开环占空比
				...
				CLAMP(tmp,10,1000);
				OpenLoopPWMval=tmp;
				sciPutString(1,"设置开环占空比:",0);
				send_number_to_USART(tmp);
				break;
			...
			case 0xC1:
				if(Mark.Status_Bits.MotorState == M_STOP)
				{
					if((SCI_Msg.rxData[1]>=3)||(SCI_Msg.rxData[1]<=5))
						sciPutString(1,"设置电机转动模式，开环03，速度闭环04，电流闭环05",0);
